...
class trainer:
    ...
    def __init__(self, state):
        ...
        if state == "activeTrain":
            self.activeTrain(int(input("Input number of simulated rounds:")))
        ...
        for episode in range(numOfEpisodes):
            ...
            while True:
                ...
                if randomInteger > 511:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[0]]
                elif randomInteger > 255:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[1]]
                elif randomInteger > 127:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[2]]
                elif randomInteger > 63:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[3]]
                elif randomInteger > 31:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[4]]
                elif randomInteger > 15:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[5]]
                elif randomInteger > 7:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[6]]
                elif randomInteger > 3:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[7]]
                elif randomInteger > 1:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[8]]
                elif randomInteger == 1:
                    action = self.ANNCAv2.chessLogic.actionSpace[topIndices[9]]
     
                
                observation, reward, done, info, switchPlayer, startLocation, endLocation = self.ANNCAv2.chessLogic.step(action)

                if switchPlayer:
                    self.ANNCAv2.chessLogic.render()
                ...
                for move in game.mainline_moves():
                    ...
                    if(whiteTurn):
                        ...
                        if winner == 'W':
                            ...
                            self.ANNCAv2.chessLogic.trainANN(reward, qValues, actionIndex)
    ...
    def preprocessOutput(self,sourceMove,targetMove,inputBoard, playerColor):
        ...
        if playerColor == "W":
            sourceCoords = positionToCoordinatesWhite[sourceMove]
            targetCoords = positionToCoordinatesWhite[targetMove]
            piece = inputBoard[sourceCoords[0]][sourceCoords[1]]

            action = (piece,(targetCoords[1]-sourceCoords[1],sourceCoords[0]-targetCoords[0]))
        else:
            sourceCoords = positionToCoordinatesBlack[sourceMove]
            targetCoords = positionToCoordinatesBlack[targetMove]
            piece = inputBoard[sourceCoords[0]][sourceCoords[1]]
